...
public:
//32768
  const static int AIRSPY_FRAMES = 32768; // number of complex samples to process per buffer cycle. 
  ...
  const static int SAMPLE_SCALE = 16;  // Airspy provides signed 16-bit packets that have 12 bit of resolution so scaling factor ends up being the same as rtlsdr.
                                       // amount by which to multiply signed 12-bit samples to get signed 16-bit sample; for plugins, this
